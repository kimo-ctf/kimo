...
func init() {
	...
	utilruntime.Must(kimov1alpha1.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "d8ccebf9.kimo.io",
	...
	if err := (&controller.ChallengeTemplateReconciler{
		...
		setupLog.Error(err, "Failed to create controller", "controller", "challengetemplate")
		os.Exit(1)
	}
	...
	if err := (&controller.ChallengeSetReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "challengeset")
		os.Exit(1)
	}
	if err := (&controller.NetworkFenceReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "networkfence")
		os.Exit(1)
	}
